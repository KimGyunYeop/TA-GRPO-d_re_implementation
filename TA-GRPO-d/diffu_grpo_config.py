...
class DiffuGRPOConfig(TrainingArguments):
    ...
    use_trajectory_aware_rewards: bool = field(
        default=False,
        metadata={
            "help": "Whether to use trajectory-aware rewards, which take into account the entire trajectory of generations. "
            "If set to `True`, the reward functions should return a list of rewards for each step in the trajectory."
        },
    )
    prob_threshold: float = field(
        default=0.7,
        metadata={"help": "Threshold for selecting tokens to remask based on confidence."},
    )
    prob_delta: float = field(
        default=0.5,
        metadata={"help": "Delta value for adjusting the confidence threshold."},
    )
    prob_min_gen: int = field(
        default=1,
        metadata={"help": "Minimum number of tokens to generate."},
    )
    delta_scale: float = field(
        default=1.0,
        metadata={"help": "Scaling factor for the delta in advantage scaling."},
    )

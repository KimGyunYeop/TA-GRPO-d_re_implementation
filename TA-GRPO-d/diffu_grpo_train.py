...
def main(grpo_config, model_config):
    ...
    if grpo_config.dataset == "code":
        dataset = dataset.map(lambda ex: {"prompt_len": len(tokenizer.apply_chat_template(ex["prompt"], tokenize=True))})
    else:
    ...
    elif grpo_config.dataset in ["code"]:
        train_set = dataset.filter(lambda ex: ex["prompt_len"] <= 200)
    else:
        train_set = dataset
    ...
    if "dream" in grpo_config.model_path.lower():
        grpo_config.mask_id = model.config.mask_token_id
        # grpo_config.block_length = grpo_config.max_completion_length
    
    import json
    os.makedirs(grpo_config.output_dir, exist_ok=False)
    with open(os.path.join(grpo_config.output_dir, "train_config.json"), "w") as f:
        json.dump(grpo_config.to_dict(), f, indent=4)
    ...
    trainer = DiffuGRPOTrainer(
        ...
        processing_class=tokenizer if "dream" in grpo_config.model_path.lower() else None,
    )
...
if __name__ == "__main__":
    ...
    # wandb.init(project=f"diffusion_route_grpo{grpo_config.dataset}")
    import os
    os.environ["WANDB_PROJECT"] = f"_{grpo_config.model_path.split('/')[-1].split('-')[0]}_TA_GRPO_{grpo_config.dataset}_REIMPLEMENT"
